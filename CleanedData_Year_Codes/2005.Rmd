---
title: "Data Cleaning Report for FBI 2005"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2005.xls"
# skip titles. In "2005.xls", there are three rows for useless titles. 
fbi2005 <- readxl::read_excel(path, skip = 3) 
```

In "2005.xls", the first three rows are titles. 

1. "Table 5": useless
2. "Crime in the United States": useless
3. "by State, 2005": the only useful information is "2005", but it has been captured by the name of our RData `fbi2005`.

Also, we notice that after the 506th row are footnotes. 

```{r}
fbi2005$State %>% str_which("1")
fbi2005[506:510,]
fbi2005 <- fbi2005[1:506, 1:13]
```
...
2. Limited data for 2005 were available for Illinois.  See Data Declaration.  
...
names(fbi2005)  <- c("State","Area", "index", "Population", "ViolentCrimeLegacy", 
...
```{r}
fbi2005$State %>% unique %>% na.omit %>% length # str_detect("PUERTO") %>% any
fbi2005_1 <- fbi2005 %>% 
...
```{r}
state <- fbi2005_1 %>% pull(State)
state[str_detect(state, "\\d$")] %>% unique # ends with a digit
```

```{r}
fbi2005_1$State <- state %>% str_remove_all("[^[:alpha:][:blank:]]") %>% 
        str_trim() %>% str_to_title()
fbi2005_1$State %>% unique # nice! 50 states + DC + Puerto Rico
```
## Pivot wider
```{r}
fbi2005_2 <- fbi2005_1 %>% 
...
fbi2005_3 %>% filter(is.na(Population) | Population == "None") %>% print(width = Inf)
popu_impute <- fbi2005_3 %>% rowid_to_column() %>% 
        filter(is.na(Population) | Population == "None", 
               AreaActuallyReporting == 1) %>% .[["rowid"]] 
popu_impute

fbi2005_3$Population[popu_impute] <- "0"
...
```{r}
fbi2005_3 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2005_3 %>% filter(Area != "State Total") %>% 
...
```
So we trace all 43 missing in est to AreaActuallyReporting = 1. 

```{r}
fbi2005_3 %>% filter(AreaActuallyReporting %>% near(1)) %>% 
...
impute <- (fbi2005_3$AreaActuallyReporting == 1) %>% replace_na(FALSE)
# we cannot impute NA due to MINNESOTA2
not_impute <- is.na(fbi2005_3$ViolentCrimeLegacy_act)
...
fbi2005_3[later_impute, ] # nice

fbi2005_4 <- fbi2005_3

fbi2005_4[impute, ] <- fbi2005_3[impute, ] %>% impute_est()
fbi2005_4[impute, ] %>% complete.cases() %>% all
```

# Split 

```{r}
fbi2005_act <- fbi2005_4 %>% 
        select(State:Population, ends_with("_act")) %>% 
        rename_with(~ str_remove(.x, "_act"), ends_with("_act"))

fbi2005_est <- fbi2005_4 %>% 
        select(State:Population, ends_with("_est")) %>% 
        rename_with(~ str_remove(.x, "_est"), ends_with("_est"))

# don't have to imput any more
fbi2005_est %>% filter(Area != "State Total") %>% complete.cases() %>% all 

save(fbi2005_act, fbi2005_est, file = "tidy_2005.RData")
```
