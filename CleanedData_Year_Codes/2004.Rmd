---
title: "Data Cleaning Report for FBI 2004"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2004.xls"
# skip titles. In "2004.xls", there are three rows for useless titles. 
fbi2004 <- readxl::read_excel(path, skip = 3) 
```
Also, we notice that after the 551th row are footnotes. 

```{r}
fbi2004$Area %>% str_which("Rate") %>% last()
fbi2004[551:555, ]
fbi2004 <- fbi2004[1:551, 1:11] %>% 
...
2. 2 Limited data for 2004 were available for Illinois and Kentucky.  See Offense Estimation, Appendix I, for details.        
...
```{r}
names(fbi2004)  <- c("index", "Population", 
                     "ViolentCrimeLegacy", 
...
```{r}
t <- fbi2004$index %>% str_remove_all("[^[:alpha:]]") %>% 
        str_detect("[:lower:]")
...
fbi2004 <- fbi2004 %>% mutate(index = parse_index(index),
...
```{r}
fbi2004_2$State %>% unique %>% length # str_detect("PUERTO") %>% any
...
```{r}
fbi2004_2$index %>% unique # correct
```  

```{r}
fbi2004_2 %>% rowid_to_column() %>% 
        filter(index == "act") %>% 
...
fbi2004_6 %>% filter(is.na(Population) | str_to_upper(Population) == "NONE") %>% print(width = Inf)
```  

```{r}
popu_impute <- fbi2004_6 %>% rowid_to_column() %>% 
        filter(is.na(Population) | Population == "None", 
               AreaActuallyReporting == 1) %>% .[["rowid"]] 
popu_impute

fbi2004_6$Population[popu_impute] <- "0"
...
```{r}
fbi2004_6 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2004_6 %>% filter(Area != "State Total") %>% 
...
```
So we trace all 49 missing in `est` to `AreaActuallyReporting` = 1. 

```{r}
fbi2004_6 %>% filter(AreaActuallyReporting %>% near(1)) %>% 
...
impute <- (fbi2004_6$AreaActuallyReporting == 1) %>% replace_na(FALSE)

fbi2004_7 <- fbi2004_6

fbi2004_7[impute, ] <- fbi2004_6[impute, ] %>% impute_est()
fbi2004_7[impute, ] %>% complete.cases() %>% all
```

# Split 

```{r}
fbi2004_act <- fbi2004_7 %>% 
        select(State:Population, ends_with("_act")) %>% 
        rename_with(~ str_remove(.x, "_act"), ends_with("_act"))

fbi2004_est <- fbi2004_7 %>% 
        select(State:Population, ends_with("_est")) %>% 
        rename_with(~ str_remove(.x, "_est"), ends_with("_est"))

# don't have to impute any more
fbi2004_est %>% filter(Area != "State Total") %>% complete.cases() %>% all

save(fbi2004_act, fbi2004_est, file = "tidy_2004.RData")
```
