---
title: "Data Cleaning Report for FBI 2011"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2011.xls"
# skip titles. In "2011.xls", there are three rows for useless titles. 
fbi2011 <- readxl::read_excel(path, skip = 3) 
```

In "2011.xls", the first three rows are titles. 

1. "Table 5": useless
2. "Crime in the United States": useless
3. "by State, 2011": the only useful information is "2011", but it has been captured by the name of our RData `fbi2011`.

Also, we notice that after the 511th row are footnotes. 

```{r}
fbi2011$State %>% str_which("1")
fbi2011[511:515,]
fbi2011 <- fbi2011[1:511, ]
```

The footnotes are copied here for future reference. 

> __Footnotes__  
1. Because of changes in the state's reporting practices, figures are not comparable to previous years' data.  
...
names(fbi2011)  <- c("State","Area", "index", "Population", "ViolentCrimeLegacy", 
...
```{r}
fbi2011_1$State <- state %>% str_remove_all("[^[:alpha:][:blank:]]") %>% 
...
fbi2011_3 %>% filter(is.na(Population) | Population == "None") %>% print(width = Inf)
popu_impute <- fbi2011_3 %>% rowid_to_column() %>% 
        filter(is.na(Population) | Population == "None", 
               AreaActuallyReporting == 1) %>% .[["rowid"]] 
popu_impute

fbi2011_3$Population[popu_impute] <- "0"
...
```{r}
fbi2011_3 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2011_3 %>% filter(Area != "State Total") %>% 
...
impute <- (fbi2011_3$AreaActuallyReporting == 1) %>% replace_na(FALSE)
# we cannot impute NA due to MINNESOTA2
not_impute <- is.na(fbi2011_3$ViolentCrimeLegacy_act)
...
fbi2011_3[later_impute, ] # nice

fbi2011_4 <- fbi2011_3

fbi2011_4[impute, ] <- fbi2011_3[impute, ] %>% impute_est()
fbi2011_4[impute, ] %>% complete.cases() %>% all
```
...
fbi2011_est %>% filter(Area != "State Total", !complete.cases(.)) %>% 
        print(width = Inf)

save(fbi2011_act, fbi2011_est, file = "tidy_2011.RData")
```
