---
title: "Data Cleaning Report for FBI 2019"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2019.xls"
# skip titles. In "2019.xls", there are three rows for useless titles. 
fbi2019 <- readxl::read_excel(path, skip = 3) 
```

In "2019.xls", the first three rows are titles. 

1. "Table 5": useless
2. "Crime in the United States": useless
3. "by State, 2018": the only useful information is "2019", but it has been captured by the name of our RData `fbi2019`.

Also, we notice that after the 506th row are footnotes. 

```{r}
fbi2019 <- fbi2019[1:510, ]
```
...
names(fbi2019)  <- c("State","Area", "index", "Population", "ViolentCrimeRevised", 
...
```{r}
state <- fbi2019_1 %>% pull(State)
state[str_detect(state, "\\d$")] %>% unique # ends with a digit
```

```{r}
fbi2019_1$State <- state %>% str_remove_all("\\d") %>% str_to_title()
fbi2019_1$State %>% unique # nice! 50 states + DC + Puerto Rico
```
## Pivot wider
```{r}
fbi2019_2 <- fbi2019_1 %>% 
...
fbi2019_3 %>% filter(is.na(Population) | Population == "None", 
               !is.na(AreaActuallyReporting)) 
fbi2019_3 %>% filter(is.na(Population))

fbi2019_3$Population <- replace_na(fbi2019_3$Population, "0")  
...
```{r}
fbi2019_3 %>% filter(Area != "State Total") %>% map_dbl(~ sum(is.na(.x))) 
```
It appears that the number of NA is either 0 or 41, and that all act columns contain no NA. That's great! But we have to check another issue: whether or not the NA in est is always associated with AreaActuallyReporting = 1. It's possible that the NA are caused by some mysterious effects. 

```{r}
fbi2019_3 %>% filter(Area != "State Total") %>% 
...
impute <- (fbi2019_3$AreaActuallyReporting == 1) %>% replace_na(FALSE)
fbi2019_4 <- fbi2019_3

fbi2019_4[impute, ] <- fbi2019_3[impute, ] %>% impute_est()
fbi2019_4[impute, ] %>% complete.cases() %>% all
```

# Split 

```{r}
fbi2019_act <- fbi2019_4 %>% 
        select(State:Population, ends_with("_act")) %>% 
        rename_with(~ str_remove(.x, "_act"), ends_with("_act"))
        

fbi2019_est <- fbi2019_4 %>% 
        select(State:Population, ends_with("_est")) %>% 
        rename_with(~ str_remove(.x, "_est"), ends_with("_est"))

fbi2019_est %>% filter(Area != "State Total") %>% complete.cases() %>% all

save(fbi2019_act, fbi2019_est, file = "tidy_2019.RData")
```
