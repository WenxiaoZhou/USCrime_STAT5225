---
title: "Data Cleaning Report for FBI 2001"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2001.xls"
# skip titles. In "2001.xls", there are four rows for useless titles. 
fbi2001 <- readxl::read_excel(path, skip = 4) 
```
Also, we notice that after the 655th row are footnotes. 

```{r}
fbi2001$Area %>% str_which("Rate") %>% last()
fbi2001[655:660, ]
fbi2001 <- fbi2001[1:655, -c(4, 11, 15)]
```  
...
> __Footnotes__  
1. The murder and nonnegligent homicides that occurred as a result of the events of September 11, 2001, were not included in this table.  See special report, Section V.    
...
3. Violent crimes are offenses of murder, forcible rape, robbery, and aggravated assault. Property crimes are offenses of burglary, larceny-theft, and motor vehicle theft.  
4. Includes offenses reported by the Zoological Police.  
5. Limited data for 2001 were available for Illinois and Kentucky; therefore, it was necessary that their crime counts be estimated.  See Offense Estimation, Appendix I, for details.      
6. Offense totals are based on all reporting agencies and estimates for unreported areas.

		
```{r}
names(fbi2001)  <- c("index", "Population", "CrimeIndex",
...
```{r}
t <- fbi2001$index %>% str_remove_all("[^[:alpha:]]") %>% 
        str_detect("[:lower:]")
...
fbi2001 <- fbi2001 %>% mutate(index = parse_index(index),
...
```{r}
fbi2001_3$diff_crime %>% na.omit() %>% near(0, tol = 1.01) %>% all
```


```{r}
fbi2001_3 <- fbi2001_3 %>% select(!c(CrimeIndex:diff_crime))
```
...
```{r}
fbi2001_4$diff_v %>% na.omit() %>% near(0, tol = 0.11) %>% all # good 
```


```{r}
fbi2001_4 <- fbi2001_4 %>% select(!diff_v)
```
...
```{r}
fbi2001_5$diff_p %>% na.omit() %>% near(0, tol = 0.11) %>% all # good
```


```{r}
fbi2001_5 <- fbi2001_5 %>% select(!c(diff_p, id))
```
...
```{r}
fbi2001_5$State %>% unique %>% length # str_detect("PUERTO") %>% any
...
> 4. An aggregated Delaware state total for forcible rape for 2001 was supplied by the Delaware State Bureau of Investigation.  

```{r}
fbi2001_5$index %>% unique # correct
```  

```{r}
fbi2001_5 %>% rowid_to_column() %>% 
        filter(index == "act") %>% 
...
fbi2001_6 %>% filter(is.na(Population) | str_to_upper(Population) == "NONE") %>% print(width = Inf)
```  

```{r}
popu_impute <- fbi2001_6 %>% rowid_to_column() %>% 
        filter(is.na(Population) | Population == "None", 
               AreaActuallyReporting == 1) %>% .[["rowid"]] 
popu_impute

fbi2001_6$Population[popu_impute] <- "0"
...
```{r}
fbi2001_6 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2001_6 %>% filter(Area != "State Total") %>% 
...
```
So we trace all 42 missing in `est` to `AreaActuallyReporting` = 1. 

```{r}
fbi2001_6 %>% filter(AreaActuallyReporting %>% near(1)) %>% 
...
impute <- (fbi2001_6$AreaActuallyReporting == 1) %>% replace_na(FALSE)

fbi2001_7 <- fbi2001_6

fbi2001_7[impute, ] <- fbi2001_6[impute, ] %>% impute_est()
fbi2001_7[impute, ] %>% complete.cases() %>% all
```

# Split 

```{r}
fbi2001_act <- fbi2001_7 %>% 
        select(State:Population, ends_with("_act")) %>% 
        rename_with(~ str_remove(.x, "_act"), ends_with("_act"))

fbi2001_est <- fbi2001_7 %>% 
        select(State:Population, ends_with("_est")) %>% 
        rename_with(~ str_remove(.x, "_est"), ends_with("_est"))

# don't have to impute any more
fbi2001_est %>% filter(Area != "State Total") %>% complete.cases() %>% all

save(fbi2001_act, fbi2001_est, file = "tidy_2001.RData")
```
