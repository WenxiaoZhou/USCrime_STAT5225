---
title: "Data Cleaning Report for FBI 2014"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2014.xls"
# skip titles. In "2014.xls", there are three rows for useless titles. 
fbi2014 <- readxl::read_excel(path, skip = 3) 
```

In "2014.xls", the first three rows are titles. 

1. "Table 5": useless
2. "Crime in the United States": useless
3. "by State, 2014": the only useful information is "2014", but it has been captured by the name of our RData `fbi2014`.

Also, we notice that after the 505th row are footnotes. 

```{r}
fbi2014$State %>% str_which("1")
fbi2014[505:508,]
fbi2014 <- fbi2014[1:505, 1:14]
```
...
names(fbi2014)  <- c("State","Area", "index", "Population", "ViolentCrimeRevised", 
...
```{r}
state <- fbi2014_1 %>% pull(State)
state[str_detect(state, "\\d$")] %>% unique # ends with a digit
```

```{r}
fbi2014_1$State <- state %>% str_remove_all("[^[:alpha:][:blank:]]") %>% 
        str_trim() %>% str_to_title()
fbi2014_1$State %>% unique # nice! 50 states + DC + Puerto Rico
```
## Pivot wider
```{r}
fbi2014_2 <- fbi2014_1 %>% 
...
fbi2014_3 %>% filter(is.na(Population) | Population == "None") %>% print(width = Inf)
popu_impute <- fbi2014_3 %>% rowid_to_column() %>% 
        filter(is.na(Population) | Population == "None", 
               AreaActuallyReporting == 1) %>% .[["rowid"]] 
popu_impute

fbi2014_3$Population[popu_impute] <- "0"
...
```{r}
fbi2014_3 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2014_3 %>% filter(Area != "State Total") %>%
...
impute <- (fbi2014_3$AreaActuallyReporting == 1) %>% replace_na(FALSE)
fbi2014_4 <- fbi2014_3

fbi2014_4[impute, ] <- fbi2014_3[impute, ] %>% impute_est()
fbi2014_4[impute, ] %>% complete.cases() %>% all
```

# Split 

```{r}
fbi2014_act <- fbi2014_4 %>% 
        select(State:Population, ends_with("_act")) %>% 
        rename_with(~ str_remove(.x, "_act"), ends_with("_act"))

fbi2014_est <- fbi2014_4 %>% 
        select(State:Population, ends_with("_est")) %>% 
        rename_with(~ str_remove(.x, "_est"), ends_with("_est"))

fbi2014_est[, -(1:3)] %>% complete.cases() %>% all

save(fbi2014_act, fbi2014_est, file = "tidy_2014.RData")
```
