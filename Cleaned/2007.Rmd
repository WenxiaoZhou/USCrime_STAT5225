---
title: "Data Cleaning Report for FBI 2007"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2007.xls"
# skip titles. In "2007.xls", there are three rows for useless titles. 
fbi2007 <- readxl::read_excel(path, skip = 3) 
```

In "2007.xls", the first three rows are titles. 

1. "Table 5": useless
2. "Crime in the United States": useless
3. "by State, 2007": the only useful information is "2007", but it has been captured by the name of our RData `fbi2007`.

Also, we notice that after the 495th row are footnotes. 

```{r}
fbi2007$State %>% str_which("1")
fbi2007[495:500,]
fbi2007 <- fbi2007[1:495, 1:13]
```
...
names(fbi2007)  <- c("State","Area", "index", "Population", "ViolentCrimeLegacy", 
...
```{r}
fbi2007$State %>% unique %>% na.omit %>% length # str_detect("PUERTO") %>% any
fbi2007_1 <- fbi2007 %>% 
...
```{r}
fbi2007_1$State <- state %>% str_remove_all("[^[:alpha:][:blank:]]") %>% 
...
fbi2007_3 %>% filter(is.na(Population) | Population == "None") %>% print(width = Inf)
popu_impute <- fbi2007_3 %>% rowid_to_column() %>% 
        filter(is.na(Population) | Population == "None", 
               AreaActuallyReporting == 1) %>% .[["rowid"]] 
popu_impute

fbi2007_3$Population[popu_impute] <- "0"
...
```{r}
fbi2007_3 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2007_3 %>% filter(Area != "State Total") %>% 
        filter(is.na(ViolentCrimeLegacy_act), is.na(ViolentCrimeLegacy_est)) 
```
Two missing are from MINNESOTA3.
...
```
So we trace 53 missing in est to AreaActuallyReporting = 1 and 2 to MINNESOTA3. 
...
impute <- (fbi2007_3$AreaActuallyReporting == 1) %>% replace_na(FALSE)
# we cannot impute NA due to MINNESOTA2
not_impute <- is.na(fbi2007_3$ViolentCrimeLegacy_act)
...
fbi2007_3[later_impute, ] # nice

fbi2007_4 <- fbi2007_3

fbi2007_4[impute, ] <- fbi2007_3[impute, ] %>% impute_est()
fbi2007_4[impute, ] %>% complete.cases() %>% all
```
...
fbi2007_est %>% filter(Area != "State Total", !complete.cases(.)) %>% 
        print(width = Inf)

save(fbi2007_act, fbi2007_est, file = "tidy_2007.RData")
```
