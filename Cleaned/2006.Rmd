---
title: "Data Cleaning Report for FBI 2006"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2006.xls"
# skip titles. In "2006.xls", there are three rows for useless titles. 
fbi2006 <- readxl::read_excel(path, skip = 3) 
```

In "2006.xls", the first three rows are titles. 

1. "Table 5": useless
2. "Crime in the United States": useless
3. "by State, 2006": the only useful information is "2006", but it has been captured by the name of our RData `fbi2006`.

Also, we notice that after the 501th row are footnotes. 

```{r}
fbi2006$State %>% str_which("1")
fbi2006[501:505,]
fbi2006 <- fbi2006[1:501, 1:13]
```
...
names(fbi2006)  <- c("State","Area", "index", "Population", "ViolentCrimeLegacy", 
...
```{r}
fbi2006$State %>% unique %>% na.omit %>% length # str_detect("PUERTO") %>% any
fbi2006_1 <- fbi2006 %>% 
...
```{r}
fbi2006_1$State <- state %>% str_remove_all("[^[:alpha:][:blank:]]") %>% 
...
fbi2006_3 %>% filter(is.na(Population) | Population == "None") %>% print(width = Inf)
popu_impute <- fbi2006_3 %>% rowid_to_column() %>% 
        filter(is.na(Population) | Population == "None", 
               AreaActuallyReporting == 1) %>% .[["rowid"]] 
popu_impute

fbi2006_3$Population[popu_impute] <- "0"
...
```{r}
fbi2006_3 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2006_3 %>% filter(Area != "State Total") %>% 
...
```
So we trace 47 missing in est to AreaActuallyReporting = 1 and 3 to MINNESOTA3. 
...
impute <- (fbi2006_3$AreaActuallyReporting == 1) %>% replace_na(FALSE)
# we cannot impute NA due to MINNESOTA2
not_impute <- is.na(fbi2006_3$ViolentCrimeLegacy_act)
...
fbi2006_3[later_impute, ] # nice

fbi2006_4 <- fbi2006_3

fbi2006_4[impute, ] <- fbi2006_3[impute, ] %>% impute_est()
fbi2006_4[impute, ] %>% complete.cases() %>% all
```
...
fbi2006_est %>% filter(Area != "State Total", !complete.cases(.)) %>% 
        print(width = Inf)

save(fbi2006_act, fbi2006_est, file = "tidy_2006.RData")
```
