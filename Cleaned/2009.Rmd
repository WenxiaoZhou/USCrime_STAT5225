---
title: "Data Cleaning Report for FBI 2009"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2009.xls"
# skip titles. In "2009.xls", there are three rows for useless titles. 
fbi2009 <- readxl::read_excel(path, skip = 3) 
```

In "2009.xls", the first three rows are titles. 

1. "Table 5": useless
2. "Crime in the United States": useless
3. "by State, 2009": the only useful information is "2009", but it has been captured by the name of our RData `fbi2009`.

Also, we notice that after the 495th row are footnotes. 

```{r}
fbi2009$State %>% str_which("1")
fbi2009[495:500,]
fbi2009 <- fbi2009[1:495, 1:13]
```
...
2. Limited data for 2009 were available for Illinois.  See Data Declaration. 
3  The data collection methodology for the offense of forcible rape used by the Illinois and the Minnesota state Uniform Crime Reporting (UCR) Programs (with the exception of Rockford, IL, and Minneapolis and St. Paul, MN) does not comply with national UCR guidelines.  Consequently, their state figures for forcible rape (with the exception of Rockford, IL, and Minneapolis and St. Paul, MN) have been estimated for inclusion in this table.  Table 8, Offenses Known to Law Enforcement, provides the reported female forcible rape crime figure.  
4. Because of changes in the state's reporting practices, figures are not comparable to previous years' data. 
...
names(fbi2009)  <- c("State","Area", "index", "Population", "ViolentCrimeLegacy", 
...
```{r}
fbi2009$State %>% unique %>% na.omit %>% length # str_detect("PUERTO") %>% any
fbi2009_1 <- fbi2009 %>% 
...
```{r}
fbi2009_1$State <- state %>% str_remove_all("[^[:alpha:][:blank:]]") %>% 
...
fbi2009_3 %>% filter(is.na(Population) | Population == "None") %>% print(width = Inf)
popu_impute <- fbi2009_3 %>% rowid_to_column() %>% 
        filter(is.na(Population) | Population == "None", 
               AreaActuallyReporting == 1) %>% .[["rowid"]] 
popu_impute

fbi2009_3$Population[popu_impute] <- "0"
...
```{r}
fbi2009_3 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2009_3 %>% filter(Area != "State Total") %>% 
...
impute <- (fbi2009_3$AreaActuallyReporting == 1) %>% replace_na(FALSE)
# we cannot impute NA due to MINNESOTA2
not_impute <- is.na(fbi2009_3$ViolentCrimeLegacy_act)
...
fbi2009_3[later_impute, ] # nice

fbi2009_4 <- fbi2009_3

fbi2009_4[impute, ] <- fbi2009_3[impute, ] %>% impute_est()
fbi2009_4[impute, ] %>% complete.cases() %>% all
```
...
fbi2009_est %>% filter(Area != "State Total", !complete.cases(.)) %>% 
        print(width = Inf)

save(fbi2009_act, fbi2009_est, file = "tidy_2009.RData")
```
