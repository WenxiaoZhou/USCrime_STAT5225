---
title: "Data Cleaning Report for FBI 2015"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2015.xls"
# skip titles. In "2015.xls", there are three rows for useless titles. 
fbi2015 <- readxl::read_excel(path, skip = 3) 
```

In "2015.xls", the first three rows are titles. 

1. "Table 5": useless
2. "Crime in the United States": useless
3. "by State, 2015": the only useful information is "2015", but it has been captured by the name of our RData `fbi2015`.

Also, we notice that after the 502th row are footnotes. 

```{r}
fbi2015$State %>% str_which("1")
fbi2015[502:505,]
fbi2015 <- fbi2015[1:502, 1:14]
```
...
names(fbi2015)  <- c("State","Area", "index", "Population", "ViolentCrimeRevised", 
...
```{r}
state <- fbi2015_1 %>% pull(State)
state[str_detect(state, "\\d$")] %>% unique # ends with a digit
```

```{r}
fbi2015_1$State <- state %>% str_remove_all("\\d") %>% str_to_title()
fbi2015_1$State %>% unique # nice! 50 states + DC + Puerto Rico
```
## Pivot wider
```{r}
fbi2015_2 <- fbi2015_1 %>% 
...
fbi2015_3 %>% filter(is.na(Population) | Population == "None") %>% print(width = Inf)
popu_impute <- fbi2015_3 %>% rowid_to_column() %>% 
        filter(Population == "None", AreaActuallyReporting == 1) %>% .[["rowid"]] 

fbi2015_3$Population[popu_impute] <- "0"
...
```{r}
fbi2015_3 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2015_3 %>% filter(Area != "State Total") %>% 
        filter(is.na(AreaActuallyReporting)) %>% print(width = Inf)
```
Hawaii's nonmetropolitan counties only report estimated total and no actual data. (remove NA after spliting)
...
impute <- (fbi2015_3$AreaActuallyReporting == 1) %>% replace_na(FALSE)
fbi2015_4 <- fbi2015_3

fbi2015_4[impute, ] <- fbi2015_3[impute, ] %>% impute_est()
fbi2015_4[impute, ] %>% complete.cases() %>% all
```

# Split 

```{r}
fbi2015_act <- fbi2015_4 %>% 
        select(State:Population, ends_with("_act")) %>% 
        rename_with(~ str_remove(.x, "_act"), ends_with("_act"))

fbi2015_est <- fbi2015_4 %>% 
        select(State:Population, ends_with("_est")) %>% 
        rename_with(~ str_remove(.x, "_est"), ends_with("_est"))

fbi2015_est[, -(1:3)] %>% complete.cases(.) %>% all

save(fbi2015_act, fbi2015_est, file = "tidy_2015.RData")
```
