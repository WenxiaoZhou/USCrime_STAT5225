---
title: "Data Cleaning Report for FBI 2000"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2000.xls"
# skip titles. In "2000.xls", there are three rows for useless titles. 
fbi2000 <- readxl::read_excel(path, skip = 3) 
```
Also, we notice that after the 632th row are footnotes. 

```{r}
fbi2000$Area %>% str_which("Rate") %>% last()
fbi2000[632:635, ]
fbi2000 <- fbi2000[1:632, -c(4, 11, 15)]
```  
...
4. An aggregated Delaware state total for forcible rape for 2000 was supplied by the Delaware State Bureau of Investigation.  
5. Includes offenses reported by the Zoological Police.  
6. Limited data for 2000 were available for Illinois, Kansas, Kentucky, and Montana; therefore, it was necessary  that their crime counts be estimated. See Offense Estimation, Appendix I, for details.    
...
```{r}
t <- fbi2000$index %>% str_remove_all("[^[:alpha:]]") %>% 
        str_detect("[:lower:]")
...
fbi2000 <- fbi2000 %>% mutate(index = parse_index(index),
...
fbi2000_3 %>% 
        ...
        print() %>% pull(id) -> t3
```  
Well, -1 rounding error is acceptable. 

```{r}
fbi2000_3$diff_crime %>% na.omit() %>% near(0, tol = 1.01) %>% all
```


```{r}
fbi2000_3 <- fbi2000_3 %>% select(!c(CrimeIndex:diff_crime))
```
...
```{r}
fbi2000_4$diff_v %>% na.omit() %>% near(0, tol = 0.11) %>% all # good 
```


```{r}
fbi2000_4 <- fbi2000_4 %>% select(!diff_v)
```
...
```{r}
fbi2000_5$diff_p %>% na.omit() %>% near(0, tol = 0.11) %>% all # good
```


```{r}
fbi2000_5 <- fbi2000_5 %>% select(!c(diff_p, id))
```
...
```{r}
fbi2000_5$State %>% unique %>% length # str_detect("PUERTO") %>% any
...
```{r}
fbi2000_5$index %>% unique # correct
```  

```{r}
fbi2000_5 %>% rowid_to_column() %>% 
        filter(index == "act") %>% 
...
fbi2000_5[t5, 5:13] <- fbi2000_5[t5, 5:13] %>% 
        replace(is.na(.), 0)
...
fbi2000_6 %>% filter(is.na(Population) | str_to_upper(Population) == "NONE") %>% print(width = Inf)
```  

```{r}
popu_impute <- fbi2000_6 %>% rowid_to_column() %>% 
        filter(is.na(Population) | Population == "None", 
               AreaActuallyReporting == 1) %>% .[["rowid"]] 
popu_impute

fbi2000_6$Population[popu_impute] <- "0"
...
```{r}
fbi2000_6 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2000_6 %>% filter(Area != "State Total") %>% 
...
```{r}
fbi2000_6 %>% filter(Area != "State Total", State != "Delaware") %>% 
...
```
So we trace 44 missing in `est` to `AreaActuallyReporting` = 1. 
...
```{r}
fbi2000_6 %>% filter(AreaActuallyReporting %>% near(1)) %>% 
...
impute <- (fbi2000_6$AreaActuallyReporting == 1) %>% replace_na(FALSE)
later_impute <- (fbi2000_6$State == "Delaware" & fbi2000_6$AreaActuallyReporting == 1) %>% 
        replace_na(FALSE)
...
fbi2000_7[impute, ] <- fbi2000_6[impute, ] %>% impute_est()
fbi2000_7[impute, ] %>% complete.cases() %>% all
```

# Split 

```{r}
fbi2000_act <- fbi2000_7 %>% 
        select(State:Population, ends_with("_act")) %>% 
        rename_with(~ str_remove(.x, "_act"), ends_with("_act"))

fbi2000_est <- fbi2000_7 %>% 
        select(State:Population, ends_with("_est")) %>% 
        rename_with(~ str_remove(.x, "_est"), ends_with("_est"))

fbi2000_est[later_impute, ] <- fbi2000_act[later_impute, ]

# don't have to impute any more
fbi2000_est %>% filter(Area != "State Total", !complete.cases(.))

save(fbi2000_act, fbi2000_est, file = "tidy_2000.RData")
```
