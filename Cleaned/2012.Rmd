---
title: "Data Cleaning Report for FBI 2012"
author: "Min Lin"
date: "2021/03/18"
header-includes:
...
path <- "C:\\Users\\Min\\OneDrive\\UConn\\Spring 2021\\BIST 5225_Data Management and Programming in R and SAS\\Group Assignment\\Crime_by_State\\2012.xls"
# skip titles. In "2012.xls", there are three rows for useless titles. 
fbi2012 <- readxl::read_excel(path, skip = 3) 
```

In "2012.xls", the first three rows are titles. 

1. "Table 5": useless
2. "Crime in the United States": useless
3. "by State, 2012": the only useful information is "2012", but it has been captured by the name of our RData `fbi2012`.

Also, we notice that after the 502th row are footnotes. 

```{r}
fbi2012$State %>% str_which("1")
fbi2012[502:505,]
fbi2012 <- fbi2012[1:502, ]
```
...
names(fbi2012)  <- c("State","Area", "index", "Population", "ViolentCrimeLegacy", 
...
```{r}
fbi2012_1$State <- state %>% str_remove_all("[^[:alpha:][:blank:]]") %>% 
...
fbi2012_3 %>% filter(is.na(Population) | Population == "None") %>% print(width = Inf)
popu_impute <- fbi2012_3 %>% rowid_to_column() %>% 
        filter(is.na(Population) | Population == "None", 
               AreaActuallyReporting == 1) %>% .[["rowid"]] 
popu_impute

fbi2012_3$Population[popu_impute] <- "0"
...
```{r}
fbi2012_3 %>% filter(Area != "State Total") %>% 
        map_dbl(~ sum(is.na(.x))) 
fbi2012_3 %>% filter(Area != "State Total") %>% 
        filter(is.na(ViolentCrimeLegacy_act), is.na(ViolentCrimeLegacy_est)) 
```
Three missing are from MINNESOTA2.
...
```
So we trace 48 missing in est to AreaActuallyReporting = 1 and 3 to MINNESOTA2. Even worse, the three rows of Minnesota are associated with AreaActuallyReporting = 1!
...
impute <- (fbi2012_3$AreaActuallyReporting == 1) %>% replace_na(FALSE)
# we cannot impute NA due to MINNESOTA2
not_impute <- is.na(fbi2012_3$ViolentCrimeLegacy_act)

later_impute <- (impute & not_impute) %>% which 
impute <- impute & (!not_impute)

# check 
fbi2012_3[later_impute, ] # nice

fbi2012_4 <- fbi2012_3

fbi2012_4[impute, ] <- fbi2012_3[impute, ] %>% impute_est()
fbi2012_4[impute, ] %>% complete.cases() %>% all
```
...
# to handle MINNESOTA2, we copy from act to est after splitting:
fbi2012_act[later_impute, ] %>% print(width = Inf)
fbi2012_est[later_impute, ] %>% print(width = Inf)

fbi2012_est[later_impute, ] <- fbi2012_act[later_impute, ]
fbi2012_est[-later_impute, -(1:3)] %>% complete.cases() %>% all

save(fbi2012_act, fbi2012_est, file = "tidy_2012.RData")
```
